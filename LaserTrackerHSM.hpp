...
namespace Events {
    ...
    struct MeasurementComplete {
        ...
        std::string operator()() const {
            std::ostringstream oss;
            oss << "MeasurementComplete: (" << std::fixed << std::setprecision(6)
                << x << ", " << y << ", " << z << ")";
            return oss.str();
        }
    };
    ...
    struct ErrorOccurred {
        ...
        std::string operator()() const {
            return "Error[" + std::to_string(errorCode) + "]: " + description;
        }
    };
...
namespace States {
    ...
    struct Off {
        ...
        void onEntry() const {
            std::cout << "  [ENTRY] Off: Laser tracker powered down\n";
        }

        void onExit() const {
            std::cout << "  [EXIT] Off: Preparing for power up\n";
        }
    };
    ...
    struct Initializing {
        ...
        void onEntry() const {
            std::cout << "  [ENTRY] Initializing: Starting self-test and calibration\n";
        }

        void onExit() const {
            std::cout << "  [EXIT] Initializing: Self-test complete\n";
        }
    ...
    struct Idle {
        ...
        void onEntry() const {
            std::cout << "  [ENTRY] Idle: Ready for operation, laser standby\n";
        }

        void onExit() const {
            std::cout << "  [EXIT] Idle: Activating laser systems\n";
        }
    };
    ...
    struct Error {
        ...
        void onEntry() const {
            std::cout << "  [ENTRY] Error: System error detected - Code "
                      << errorCode << ": " << description << "\n";
        }

        void onExit() const {
            std::cout << "  [EXIT] Error: Error cleared, resuming operation\n";
        }
    };
    ...
    struct Searching {
        ...
        void onEntry() const {
            std::cout << "  [ENTRY] Searching: Scanning for retroreflector target\n";
        }

        void onExit() const {
            std::cout << "  [EXIT] Searching: Target acquisition complete\n";
        }

        void updateSearchAngle(double angle) {
            searchAngle = angle;
            std::cout << "  [ACTION] Search angle: " << std::fixed
                      << std::setprecision(1) << searchAngle << " degrees\n";
        }
    };
    ...
    struct Locked {
        ...
        void onEntry() const {
            std::cout << "  [ENTRY] Locked: Target acquired at " << std::fixed
                      << std::setprecision(3) << targetDistance_mm << " mm\n";
        }

        void onExit() const {
            std::cout << "  [EXIT] Locked: Transitioning tracking mode\n";
        }
    };
    ...
    struct Measuring {
        ...
        void onEntry() const {
            std::cout << "  [ENTRY] Measuring: Starting precision measurement\n";
        }

        void onExit() const {
            std::cout << "  [EXIT] Measuring: Measurement session ended ("
                      << measurementCount << " points recorded)\n";
        }

        void recordMeasurement(double x, double y, double z) {
            lastX = x; lastY = y; lastZ = z;
            ++measurementCount;
            std::cout << "  [ACTION] Point #" << measurementCount << ": ("
...
public:
    ...
    bool isInState() const {
        return std::holds_alternative<S>(currentState_);
    }
    ...
    void printState() const {
        std::cout << "Current State: [" << getCurrentStateName() << "]\n";
    }
...
private:
    ...
    template<typename NewSubState>
    void transitionOperationalTo(States::Operational& op, NewSubState newSub) {
    ...
    template<typename NewSubState>
    void transitionTrackingTo(States::Tracking& tracking, NewSubState newSub) {
        // Exit current sub-state
    ...
    bool handleEvent(States::Off& state, const Event& event) {
        return std::visit([this, &state](const auto& e) -> bool {
    ...
    bool handleEvent(States::Operational& state, const Event& event) {
        ...
        bool handled = std::visit([this, &state](const auto& e) -> bool {
            using E = std::decay_t<decltype(e)>;
        ...
        return std::visit(
            [this, &state, &event](auto& subState) -> bool {
                return this->handleOperationalSubEvent(state, subState, event);
            },
            state.subState
        );
    }
    ...
    bool handleOperationalSubEvent(States::Operational& parent,
                                   States::Initializing& state,
                                   const Event& event) {
        return std::visit([this, &parent, &state](const auto& e) -> bool {
            using E = std::decay_t<decltype(e)>;
    ...
    bool handleOperationalSubEvent(States::Operational& parent,
                                   States::Idle& /*state*/,
                                   const Event& event) {
        return std::visit([this, &parent](const auto& e) -> bool {
            using E = std::decay_t<decltype(e)>;
    ...
    bool handleOperationalSubEvent(States::Operational& parent,
        ...
        bool handled = std::visit([this, &parent, &state](const auto& e) -> bool {
            using E = std::decay_t<decltype(e)>;
        ...
        return std::visit(
            [this, &parent, &state, &event](auto& subState) -> bool {
                return this->handleTrackingSubEvent(parent, state, subState, event);
            },
            state.subState
        );
    }
    ...
    bool handleTrackingSubEvent(States::Operational& /*parent*/,
                                States::Tracking& tracking,
                                States::Searching& /*state*/,
                                const Event& event) {
        return std::visit([this, &tracking](const auto& e) -> bool {
            using E = std::decay_t<decltype(e)>;
    ...
    bool handleTrackingSubEvent(States::Operational& /*parent*/,
                                States::Tracking& tracking,
                                States::Measuring& state,
                                const Event& event) {
        return std::visit([this, &tracking, &state](const auto& e) -> bool {
            using E = std::decay_t<decltype(e)>;
